...
dberr_t CatalogManager::CreateTable(const std::string& table_name, TableSchema* schema,
   ...
   /* table_info is created by CatalogManager using CatalogManager's heap_ */
    ...
    page_id_t page_id;//Table MetaData������ҳ
    ...
    //char meta[len + 1];
    char* meta = (char*)malloc(sizeof(char) * (len + 1));
    ...
    char* cmeta = (char*)malloc(sizeof(char) * (len + 1));
    //char cmeta[len + 1];
...
dberr_t CatalogManager::CreateIndex(const std::string& table_name, const std::string& index_name,
    ...
    //��index����Ϣд��
    page_id_t page_id;//����indexMetaData������ҳ
...
dberr_t CatalogManager::DropTable(const std::string& table_name) {
    ...
    for (auto it3 : it2->second) {
        ...
        indexName.push_back(it3.first);
    }
    for (auto index : indexName)
    {
        DropIndex(table_name, index);
    }
